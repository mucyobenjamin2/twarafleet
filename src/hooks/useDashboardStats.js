...
export function useDashboardStats() {
  ...
  const load = useCallback(async () => {
    ...
    const myMotoIds = fleet.map(m => m.id)
    ...
    const [
      ...
      // Kuyungurura ngo haze na versements zose z'uyu munsi
    ...
    // ⚠️ HANO: Kubara gusa amafaranga yamaze kwemezwa (approved) na Admin!
    const approvedVersements = (todaysVersements.data ?? []).filter(v => v.status === 'approved' || v.status === 'paid')
    const collectedTotal = approvedVersements.reduce((sum, v) => sum + Number(v.amount || 0), 0)
