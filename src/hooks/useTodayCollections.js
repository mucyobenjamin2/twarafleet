...
export function useTodayCollections() {
  ...
  const load = useCallback(async () => {
    ...
    const [motos, assignments, versements, nonWorking] = await Promise.all([
      supabase.from('motorcycles').select('id, plate_number, daily_target, off_day').eq('status', 'active').eq('owner_id', user.id).order('plate_number'),
    ...
    setRows((motos.data ?? []).map(m => {
      const motoOffDay = (m.off_day || 'saturday').toLowerCase()
      const isWeeklyOff = (motoOffDay === currentDayName)
      const isSpecificOff = nonWorkingSet.has(m.id)
      ...
      return {
        ...
        isNonWorking: isWeeklyOff || isSpecificOff
      }
    }))
    setLoading(false)
  }, [today])
  ...
  return { rows, loading, refresh: load, today }
}