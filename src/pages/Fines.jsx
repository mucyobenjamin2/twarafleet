...
export default function Fines() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 md:p-6">
          ...
          {selectedVehicle ? (
            <div className="space-y-2">
              <button
                ...
                className="inline-flex items-center gap-2 text-xs font-black text-emerald-700 dark:text-emerald-400 bg-paper border border-line px-3.5 py-1.5 rounded-xl hover:bg-paper-raised transition shadow-sm active:scale-95"
              >
              ...
              <h1 className="font-display text-2xl font-black text-ink flex items-center gap-2 pt-1">
                <Bike className="text-emerald-600 dark:text-emerald-400" size={28} /> {selectedVehicle.plate_number} Traffic Fines
              </h1>
            </div>
          ) : (
            <div>
              <h1 className="font-display text-2xl font-black text-ink flex items-center gap-2">
                <ShieldAlert className="text-emerald-600 dark:text-emerald-400" size={28} /> Traffic Fines Manager
              </h1>
      ...
      {/* 🏍️ VIEW 1: FLEET VEHICLES CARDS DASHBOARD (GREEN TRANSLUCENT CARDS) */}
      {!selectedVehicle ? (
        <div className="space-y-4">
          <div className="flex items-center justify-between border-b border-line pb-2">
            <h2 className="text-xs font-black uppercase tracking-wider text-ink flex items-center gap-2">
              <Bike size={16} className="text-emerald-600 dark:text-emerald-400" /> Active Motorcycles Fines Fleet ({motorcycles.length})
            </h2>
          </div>
            ...
            {motorcycles.map((moto) => {
              ...
              return (
                <div
                  ...
                  className="p-6 rounded-2xl border-2 border-emerald-500/40 bg-emerald-500/10 dark:bg-emerald-950/20 hover:border-emerald-500 transition-all shadow-sm cursor-pointer space-y-4 hover:scale-105 group relative overflow-visible flex flex-col justify-between items-center text-center backdrop-blur-md"
                >
                  ...
                  <div className="w-full space-y-2 pt-1">
                    <span className="plate inline-block text-xl sm:text-2xl font-mono font-black border border-emerald-500/30 bg-paper text-ink px-4 py-1.5 rounded-xl uppercase tracking-widest shadow-sm">
                      {moto.plate_number}
                    </span>

                    {/* DRIVER NAME */}
                    <p className="text-xs font-black text-ink-soft flex items-center justify-center gap-1 truncate">
                      <User size={13} className="text-emerald-600 dark:text-emerald-400" /> {driverName}
                    </p>
                  </div>

                  {/* BOTTOM REBA PAGE ACTION */}
                  <div className="w-full pt-2 border-t border-emerald-500/20 flex items-center justify-between text-xs font-black text-emerald-700 dark:text-emerald-400 group-hover:underline">
        ...
        <div className="space-y-6">
          ...
          {/* 🚨 AMBER PENDING APPROVAL NOTIFICATION CARD */}
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1 sm:w-64">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 rounded-xl border border-line bg-paper text-sm text-ink font-bold focus:outline-none focus:border-emerald-500 shadow-sm"
                />
              </div>

              <button
                onClick={() => setShowHistory(!showHistory)}
                className="flex items-center gap-1.5 text-xs font-black text-emerald-700 dark:text-emerald-400 bg-paper border border-line px-3 py-2 rounded-xl hover:bg-paper-raised transition shadow-sm whitespace-nowrap"
              >
                  ...
                  <tbody className="divide-y divide-line text-sm">
                    {searchFilteredFines.map(fine => (
                      <tr key={fine.id} className="hover:bg-paper/40 transition-colors">
                        ...
                        <td className="p-4 whitespace-nowrap">
                          {fine.due_date ? (
                            <span className="inline-flex items-center gap-1 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 px-2.5 py-1 rounded-lg text-xs font-mono font-bold border border-emerald-500/20">
