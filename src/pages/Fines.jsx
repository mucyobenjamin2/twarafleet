        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between border-b border-line pb-2">
            <h2 className="text-xs font-black uppercase tracking-wider text-ink flex items-center gap-2">
              <Bike size={16} className="text-sky-600 dark:text-sky-400" /> Active Motorcycles Fines Fleet ({motorcycles.length})
            </h2>
          </div>
                ...
                >
                  ...
                  <div className="w-full space-y-2 pt-1">
                    ...
                    <p className="text-xs font-black text-ink-soft flex items-center justify-center gap-1 truncate">
                      <User size={13} className="text-sky-600 dark:text-sky-400" /> {driverName}
                    </p>
                  </div>

                  <div className="w-full pt-2 border-t border-sky-500/20 flex items-center justify-between text-xs font-black text-sky-600 dark:text-sky-400 group-hover:underline">
                ...
                {pendingApprovals.map(fine => (
                  <div key={fine.id} className="p-4 rounded-xl bg-paper border border-amber-500/30 shadow-sm flex items-center justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      {fine.momo_ref && (
                        <p className="text-[10px] font-mono font-black text-sky-600 dark:text-sky-400">MoMo Ref: {fine.momo_ref}</p>
                      )}
                    </div>

                    <div className="text-right space-y-2">
                      <p className="font-mono font-black text-sky-600 dark:text-sky-400 text-sm">
                        {Number(fine.amount).toLocaleString()} RWF
                      </p>
                      <button
                        onClick={() => handleApprovePayment(fine.id)}
                        className="flex items-center gap-1 bg-sky-600 hover:bg-sky-700 text-white text-xs font-black px-3 py-1.5 rounded-lg shadow-sm transition"
                      >
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1 sm:w-64">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 rounded-xl border border-line bg-paper text-sm text-ink font-bold focus:outline-none focus:border-sky-500 shadow-sm"
                />
              </div>
                  ...
                  <tbody className="divide-y divide-line text-sm">
                    {searchFilteredFines.map(fine => (
                      <tr key={fine.id} className="hover:bg-paper/40 transition-colors">
                        ...
                        <td className="p-4 whitespace-nowrap">
                          {fine.due_date ? (
                            <span className="inline-flex items-center gap-1 bg-sky-500/10 text-sky-600 dark:text-sky-400 px-2.5 py-1 rounded-lg text-xs font-mono font-bold border border-sky-500/20">
                        ...
                        <td className="p-4 font-mono font-black text-sky-600 dark:text-sky-400 whitespace-nowrap">
                          {Number(fine.amount).toLocaleString()} RWF
                        </td>
                        <td className="p-4 whitespace-nowrap">
                        ...
                        <td className="p-4 text-right whitespace-nowrap space-x-2">
                          {fine.status !== 'approved' && (
                            <button
                              onClick={() => handleApprovePayment(fine.id)}
                              className="bg-sky-600 hover:bg-sky-700 text-white text-xs font-black px-3 py-1.5 rounded-lg shadow-sm transition"
                            >
                              Emeza Ubwishyu
                            </button>
                          )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 py-2.5 rounded-xl bg-sky-600 font-bold text-white hover:bg-sky-700 transition disabled:opacity-50"
                >
